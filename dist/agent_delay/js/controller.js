...
function increment_problem() {
	...
	reference = false;
	tutorial = false;
	cont_standard.classList.add("d-none");
	cont_lester_score.classList.add("d-none");
	if (step >= data.length) {
		$("#myModal").modal("show");
...
function evaluate_response() {
	...
	cont_standard.classList.remove("d-none");
	cont_lester_score.classList.remove("d-none");
	start();
}
...
function store_response(css_selector, weight, score, feedback, evaluation) {
	var myObj = {
		version: "CSSGame_v1.0_agentdelay_build03/09/20",
		session: session_id,
		username: username,
		gender: gender,
		age: age,
